...
public class SignUp extends Command {
    ...
    public SignUp(Console console, Client client, SessionHandler sessionHandler) {
        super("sign_up", "Регистрация аккаунта пользователя");
    ...
    public boolean apply(String[] args, User user) {
        try {
            ...
            var response = client.sendRequest(new SignUpRequest(user));
            if (!response.getErrorMessage().isEmpty()) {
                console.printError(response.getErrorMessage());
                return false;
            }
            if (response instanceof SignUpResponse) {
                console.println("Успешная регистрация");
